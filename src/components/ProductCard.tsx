...
function ProductCard({product}: Props) {
  ...
  const starIcon = <svg xmlns="http://www.w3.org/2000/svg" className="size-3 md:size-4 opacity-80" height="24px" viewBox="0 -960 960 960" width="24px" fill="#d2042d"><path d="m354-287 126-76 126 77-33-144 111-96-146-13-58-136-58 135-146 13 111 97-33 143ZM233-120l65-281L80-590l288-25 112-265 112 265 288 25-218 189 65 281-247-149-247 149Zm247-350Z"/></svg>
  const threeStar = Array.from({ length: 3 }, (_, i) => <span key={i}>{starIcon}</span>);
  const fourStar = Array.from({ length: 4 }, (_, i) => <span key={i}>{starIcon}</span>);
  const fiveStar = Array.from({ length: 5 }, (_, i) => <span key={i}>{starIcon}</span>);
  ...
  return (
    <div className="w-[190px] md:w-[300px] h-[350px] z-0 bg-[var(--white-color)] rounded-md shadow-md overflow-hidden relative trans ">
      {
      ...
      <div className="px-2 py-4">
        <div className="flex justify-between  ">
          <h2 className=" text-md md:text-lg font-bold text-[var(--black-color2)]">{product.name}</h2>
          <span className=" cursor-pointer trans" onClick={() => setLike(!isLiked)} title={!isLiked ? `Like ${product.name}`: `Unlike ${product.name}`} aria-label="Like cake">
            {!isLiked ? unlikeIcon : likeIcon}
          </span>
        </div>
        <div className="py-1">
          <div className="flex justify-between items-center">
          <p className="py-1 text-md opacity-80 md:text-lg text-[var(--black-color2)]">{product.price} fCFA</p>
